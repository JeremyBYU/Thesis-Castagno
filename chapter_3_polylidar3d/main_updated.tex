...
\begin{algorithm}[H]
    ...
    \caption{Extract Triangles from~OPC}
...
\begin{algorithm}[H]
    ...
    return $\mathcal{T}, \mathcal{T}_{map}$
    \caption{Extract Half-Edges from~OPC}
...
\begin{table}[H]
    \centering
    \caption{Execution Time Comparisons for Synthetic and Real World~Datasets} \label{table:results_fastga}%mdpi: please confirm the format of the table. %JC_Author: Acceptable format 
   
       
       % \caption{}\label{table:results_fastga_a}
       \begin{tabular}{@{}ccc@{}}
        \toprule
  ...
  %  \begin{subtable}[t]{.5\linewidth}
    %  \centering
      %  \caption{ }\label{table:results_fastga_b}
    %    \begin{tabular}{@{}ccc@{}}
     %   \toprule
...
% \begin{table}[ht]
% \centering
% \caption{Execution Timings for 100,000 Normals}\label{table:results_fastga_a}
% \begin{tabular}{@{}ccc@{}}
% \toprule
% Algorithm & Mean (ms) & Std (ms) \\ \midrule
% FastGA    & 20.0      & 0.1      \\
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Polygon extraction of rooftops from unorganized 3D point clouds generated by airborne LiDAR. %There is no explanation for (a-e) in the figure. %JC_Author: Added (a,b,c,d,e) reference
...
%     \caption{\label{fig:ch3_kitti_f}}
%   \end{subfigure}
  \caption{Polygon extraction from KITTI unorganized 3D point cloud data acquired on a moving vehicle~\cite{geiger_vision_2013}. Four scenes %mdpi: the subfigure b repeat, please add the subfigure c. %JC_Author:Completed
...
\begin{figure}[H]
  ...
  \caption{Real-time polygon extraction from an RGBD camera. %There is no explanation for (a-b) in the figure. %JC_Author: Added (a,b) reference
...
\begin{table}[H]
\centering
\caption{Mean execution timings (ms) of Polylidar3D with RGBD data collected from indoor~settings.}
\label{table:rgbd_timings}
...
Table~\ref{table:results_lapalcian} shows the results of our Laplacian filter for one and five iterations with results separated by a semicolon. Our CPU single-threaded performance is faster than Open3D. This is mostly explained by not needing to compute a vertex adjacency list. Our CPU multi-threaded results nearly reach the ideal 4X speedup in most scenarios. GPU acceleration is quite fast but has substantial overhead on the first iteration of smoothing. This is because the OPC in CPU memory must be transferred to GPU memory which is an expensive operation. This penalty is only paid once no matter how many iterations of smoothing occur.  
...
% \begin{table}[!htb]
%     \caption{Organized Point Cloud Mesh Filtering} \label{table:results_smooting}
%     \begin{subtable}[t]{.5\linewidth}
%     \centering
%     \caption{Laplacian Filtering}\label{table:results_lapalcian}
%         \begin{tabular}{@{}ccccc@{}}
%         \toprule
...
%     \begin{subtable}[t]{.5\linewidth}
%     \centering
%     \caption{Bilateral Filtering}\label{table:results_bilateral}
%         \begin{tabular}{@{}cccc@{}}
%         \toprule
